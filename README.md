...
- It is **fragile by construction — a design cost, not just bad luck.** The heavyweight path (parallel sub-agents in isolated git worktrees) hard-depends on host support for both, with no fallback — without them the run can't produce a result. It now fails *gracefully* rather than crashing: a skipped prototype is dropped, and a proposer or iterate agent that can't finalize ends the run with a clean error (the Phase-A winner is preserved) instead of aborting it. But the failure odds still rise with agent count — the surface grows with the very parallelism that is the point. The inline "sketch-measure-pick" baseline (one agent, no worktrees, no cross-agent dependency) pays none of this.
